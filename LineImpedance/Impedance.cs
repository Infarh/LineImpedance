...
namespace LineImpedance
{
    ...
    public static class Impedance
    {
        ...
        public static double SuspendedMicroStripInverted(double H, double H0, double W, double Eps)
        {
            const double en = 2.71829;
            ...
            var f_2 = f * f;
            var f_3 = f_2 * f;
            var d00 = (2359.4010 - 97.1644 * f - 5.7706 * f_2 + 11.4112 * f_3) * 1e-3;
            var d01 = (4855.9472 - 3408.5207 * f + 15296.73 * f_2 - 2418.1785 * f_3) * 1e-5;
            var d02 = (1763.34 + 961.0481 * f - 2089.28 * f_2 + 375.8805 * f_3) * 1e-5;
            var d03 = (-556.0909 - 268.6165 * f + 623.7094 * f_2 - 119.1402 * f_3) * 1e-6;
            var d10 = (219.0660 - 253.0864 * f + 208.7469 * f_2 - 27.3285 * f_3) * 1e-3;
            var d11 = (915.5589 + 338.4033 * f - 253.2933 * f_2 + 40.4745 * f_3) * 1e-3;
            var d12 = (-1957.3790 - 1170.9360 * f + 1480.8570 * f_2 - 347.6403 * f_3) * 1e-5;
            var d13 = (486.7425 + 279.8323 * f - 431.3625 * f_2 + 108.824 * f_3) * 1e-6;
            var d20 = (5602.7670 + 4403.3560 * f - 4517.034 * f_2 + 743.2717 * f_3) * 1e-5;
            var d21 = (-2823.481 - 1562.782 * f + 3646.15 * f_2 - 823.4223 * f_3) * 1e-5;
            var d22 = (253.893 + 158.5529 * f - 3235.485 * f_2 + 919.3661 * f_3) * 1e-6;
            var d23 = (-147.0235 + 62.4343 * f + 887.5211 * f_2 - 270.7555 * f_3) * 1e-7;
            var d30 = (-3170.21 - 1931.852 * f + 2715.327 * f_2 - 519.342 * f_3) * 1e-6;
            var d31 = (596.3251 + 188.1409 * f - 1741.477 * f_2 + 465.6756 * f_3) * 1e-6;
            var d32 = (124.9655 + 577.5381 * f + 1366.453 * f_2 - 481.13 * f_3) * 1e-7;
            var d33 = (-530.2099 - 2666.3520 * f - 3220.0960 * f_2 + 1324.499 * f_3) * 1e-9;

            var f1 = Sqrt(Eps) - 1;
            var h0h = H0 / H;
            var h0h_2 = h0h * h0h;
            var h0h_3 = h0h_2 * h0h;
            var c0 = d00 + d01 * h0h + d02 * h0h_2 + d03 * h0h_3;
            var c1 = d10 + d11 * h0h + d12 * h0h_2 + d13 * h0h_3;
            var c2 = d20 + d21 * h0h + d22 * h0h_2 + d23 * h0h_3;
            var c3 = d30 + d31 * h0h + d32 * h0h_2 + d33 * h0h_3;

            var wh0 = W / H0;
            var wh0_2 = wh0 * wh0;
            var wh0_3 = wh0_2 * wh0;
            var f2 = 1 / (c0 + c1 * wh0 + c2 * wh0_2 + c3 * wh0_3);
        ...
        public static double ThickCoplanar(double H, double W, double T, double G, double Eps)
        {
            ...
            if (T >= 0.1 * G)
                throw new InvalidOperationException("T < 0.1 * G")
                {
                    Data =
                    {
                        { "H", H },
        ...
        public static double Coplanar(double W, double G, double Eps) => ThickCoplanar(20 * W, W, 0.001 * G, G, Eps);

        public static double CoplanarWithGround(double H, double W, double G, double Eps)
        {
            var a = W;
            ...
            var k = a / b;
            var k_prime = Sqrt(1 - k * k);
            //var a_t=a+1.25*tf/PI*(1+Log(4*PI*a/t));
            //var b_t=b-1.25*tf/PI*(1+Log(4*PI*a/t));
            //var k_t= a_t/b_t;
            //var k_t_prime=Sqrt(1-k_t.Pow2());
            //var k1=Tanh(PI*a_t/(4*hf))/Tanh(PI*b_t/(4*hf));
            var k1 = Tanh(PI * a / (4 * H)) / Tanh(PI * b / (4 * H));
            var k1_prime = Sqrt(1 - k1 * k1);
            var k_product = Kint(k_prime) * Kint(k1) / (Kint(k) * Kint(k1_prime));
            var epsf_eff = (1 + Eps * k_product) / (1 + k_product);
            //var eps_eff = epsf_eff;

            // espf_eff_t=espf_eff-((espf_eff-1)/((b-a)*Kint(k)/(0.7*tf*2*Kint(k_prime))+1))

            return 60 * PI / Sqrt(epsf_eff) * (1 / (Kint(k) / Kint(k_prime) + Kint(k1) / Kint(k1_prime)));
        }
    }
}
